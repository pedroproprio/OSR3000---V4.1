...
void bmp_task(data_t *data)
{
    ...
    if(err != ESP_OK) ESP_LOGE(TAG, "bmp390 device read failed (%s)", esp_err_to_name(err));
...
{
    float alt;
    if (initial_temp == 0) alt = (powf(pressure/sea_pressure, exponent)-1)*298*medium_lapse_rate; // fixed 25°C 
    else alt = (powf(pressure/sea_pressure, exponent)-1)*initial_temp*medium_lapse_rate;
    if (sea_pressure == 0) alt = (powf(pressure/101325, exponent)-1)*298*medium_lapse_rate;
    return alt;
}
