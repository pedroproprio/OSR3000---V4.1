...
public:
     ...
     *
     */
    VQF(float gyrTs, float accTs=-1.0, float magTs=-1.0);
    /**
     ...
     * @param magTs sampling time of the magnetometer measurements in seconds (the value of `gyrTs` is used if set to -1)
     */
    VQF(const VQFParams& params, float gyrTs, float accTs=-1.0, float magTs=-1.0);
     ...
     */
    void updateGyr(const float gyr[3]);
    /**
     ...
     */
    void updateAcc(const float acc[3]);
    /**
     ...
     * @param acc accelerometer measurement in m/s²
     */
    void update(const float gyr[3], const float acc[3]);
    /**
     ...
     */
    void updateBatch(const float gyr[], const float acc[], const float mag[], size_t N,
     ...
     */
    void getQuat3D(float out[4]) const;
    /**
     ...
     */
    float getBiasEstimate(float out[3]) const;
    /**
     ...
     */
    void setBiasEstimate(float bias[3], float sigma=-1.0);
    /**
     ...
     */
    void setMagRef(float norm, float dip);
     ...
     */
    void setRestDetectionThresholds(float thGyr, float thAcc);
     ...
     */
    static void quatApplyDelta(float q[4], float delta, float out[4]);
    /**
     ...
     */
    static void clip(float vec[], size_t N, float min, float max);
    /**
     ...
     */
    static float gainFromTau(float tau, float Ts);
    /**
     ...
     */
    static void filterCoeffs(float tau, float Ts, double outB[3], double outA[2]);
    /**
     ...
     */
    static void filterInitialState(float x0, const double b[3], const double a[2], double out[2]);
    /**
     ...
     */
    static void filterAdaptStateForCoeffChange(float last_y[], size_t N, const double b_old[3],
     ...
     */
    static float filterStep(float x, const double b[3], const double a[2], double state[2]);
    /**
     ...
     */
    static void filterVec(const float x[], size_t N, float tau, float Ts, const double b[3],
     ...
     */
    static void matrix3SetToScaledIdentity(float scale, float out[9]);
    /**
     ...
     */
    static void matrix3Multiply(const float in1[9], const float in2[9], float out[9]);
    /**
     ...
     * @param out output 3x3 matrix \f$\mathbf{M}_\mathrm{out}\f$ (stored in row-major order)
     */
    static void matrix3MultiplyTpsFirst(const float in1[9], const float in2[9], float out[9]);
    /**
     ...
     * @param out output 3x3 matrix \f$\mathbf{M}_\mathrm{out}\f$ (stored in row-major order)
     */
    static void matrix3MultiplyTpsSecond(const float in1[9], const float in2[9], float out[9]);
    /**
     ...
     * @param out output 3x3 matrix \f$\mathbf{M}_\mathrm{out}\f$ (stored in row-major order)
     */
    static bool matrix3Inv(const float in[9], float out[9]);
#endif
